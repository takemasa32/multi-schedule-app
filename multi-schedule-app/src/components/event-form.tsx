    ...
    <div className="card bg-base-100 shadow-xl max-w-3xl mx-auto mt-8">
      <div className="card-body">
            ...
            {dates.map((date, index) => (
              <div key={index} className="mb-3">
                <div className="flex items-center mb-1">
                  <input
                    type="datetime-local"
                    className={`input input-bordered flex-1 ${fieldErrors[`date-${index}`] ? 'input-error' : ''}`}
                    value={date}
                ...
                {fieldErrors[`date-${index}`] && (
                  <p className="text-error text-xs mt-1">{fieldErrors[`date-${index}`]}</p>
                )}
              </div>
            ))}
            <button
        ...
        <div className="text-xs text-gray-500 mt-6">
          ...
          <p className="mt-1">* 候補日程は後から追加することはできませんので、必要な候補日をすべて入力してください。</p>
        </div>
      </div>
    </div>
  );
