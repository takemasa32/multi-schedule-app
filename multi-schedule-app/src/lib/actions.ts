...
export async function createEvent(formData: FormData) {
  // 変数をtry-catchブロックの外で宣言して、スコープ問題を解決
  let publicToken = "";
  let adminToken = "";
  ...
  try {
    ...
    const dates = formData
      .getAll("dates")
      .filter((date) => date && String(date).trim() !== "") as string[];
    ...
    if (!dates.length) {
      throw new Error("少なくとも1つの候補日を入力してください。");
    }
    ...
    // 日程候補の作成
    const dateInserts = dates.map((dateString) => ({
      event_id: event.id,
      date_time: new Date(dateString),
    }));
    ...
    if (datesError) {
      console.error("日程候補作成エラー:", datesError);
      throw new Error("日程候補の登録に失敗しました。");
    }

    // ここではリダイレクトせずに、変数に値を設定するだけ
  } catch (error) {
