# **AI 駆動の開発方針に基づく Web アプリケーション仕様書**
...
- 候補日程の登録: 日付または日時を複数追加・削除できる UI。最低 1 つ以上の候補日を入力必須。主催者は必要に応じて日時のフォーマット（例: YYYY/MM/DD HH:MM）で候補を指定。
- 公開リンク共有: 作成後に表示されるイベント公開リンクをコピーして参加者に共有可能。参加者はこのリンクからイベント詳細ページにアクセスできます。
- 参加者の回答機能: 参加者がイベントページ上で自分の名前を入力し、各候補日に対して参加可能かどうかを回答できます（UI 上はチェックボックスや ○× 選択等で実装）。回答はサーバー経由で即時に DB へ保存されます。
- 回答状況の集計表示: イベントページにて、現在の各候補日の「○ 人数/× 人数」や、各参加者ごとの回答状況一覧をリアルタイムまたはページ再読み込みにより確認できます。主催者・参加者ともに他の人の回答状況を見て、自分の予定調整に役立てられます。
- 日程の最終確定機能: 主催者が回答状況を踏まえて 1 つの候補日を選びイベント日程を確定できます。確定操作により、そのイベントのステータスが「確定済み」に切り替わり、以降参加者は回答の編集ができなくなります。
...
A[イベント作成ページ] -- 作成（送信） --> B[イベント詳細ページ<br/>(日程回答フォーム)]
...
- イベント詳細ページ (Event Detail / Poll Page): 主催者がイベント作成後に遷移するページ、また参加者が公開リンクからアクセスするページです。URL は/event/[公開トークン]形式で、ページを開くとサーバーサイドで該当トークンのイベント情報を読み込み表示します。主要要素:
  ...
  -
  - 主催者用機能 UI: このページを主催者（管理者権限）として開いている場合のみ表示されます（管理者判定は後述の admin トークンで実施）。
...
3. event_dates（イベント候補日程）
   ...
   - date_time (timestamp): 候補の日時。日付のみの場合は 00:00 時刻や終日扱いで格納。タイムゾーンは UTC に統一し、必要なら別途 TZ 情報を持つか、ユーザーに合わせクライアント表示時に変換します（簡易のため UTC 前提）。
...
- ユニットテスト: ロジック部分（例えば日付フォーマット関数、トークン生成関数など）に関しては Jest もしくは Vitest を用いたユニットテストを書きます。とりわけ、カレンダー連携用の日時変換や、RLS ポリシーに関連する関数など、バグると影響が大きい箇所を重点的にテストします。個人開発のため網羅的には行いませんが、主要な機能については ChatGPT にも協力させてテストケースを洗い出し、極力自動化します。
- 結合テスト: Next.js + Supabase といった外部サービス込みの結合テストは環境構築が難しいため、ステージング環境（ローカルやテスト用 Supabase プロジェクト）での手動検証中心とします。ただし、Server Action 経由のフォーム送信が期待通り動くかなどは Playwright 等のエンドツーエンドテストツールでスクリプト化を検討します。例えば Playwright でブラウザを自動操作し、イベントを作成して回答し確定する一連の操作が成功するかをテストできます。これを GitHub Actions 上で動かし、自動テストを無料で回すことも可能です。
- 手動テスト: UI/UX の部分は人間の目で確認します。様々なデバイス幅でレイアウト崩れがないか、フォーム入力のバリデーションメッセージが適切かなど、開発者自身がシナリオを追って検証します。特にサーバーと DB の連携部分は、異常系（入力不足、同名ユーザー重複など）をシミュレートして問題がないことを確認します。
- AI によるテスト生成: 必要に応じて ChatGPT に「考えられるテストケース」を列挙させ、抜け漏れを補います。例えば「イベント確定後に再度回答できないこと」や「非常に長いイベントタイトルでも UI が崩れないこと」など、人力では見落としがちなケースにも目を向けます。
- CI 連携: GitHub にリポジトリを置き、GitHub Actions の無料枠でプルリクエスト時に Lint チェック・ビルド・ユニットテストを自動実行します。これにより、ローカルで見逃したエラーを事前に検知します。個人開発ですが CI を挟むことで安心感を高めます。
