...
- 候補日程追加UIを開いている間は管理セクション内で追加フォームを全幅表示し、日程確定の情報は一時的に隠す。追加方式は「同じ時間割で追加」「日時を個別に選ぶ」の目的ベースの文言と `aria-pressed` で状態を示す。
- 自動延長の初期日は既存候補の翌日とし、展開直後から追加日数・追加枠数と実行結果を確認できるようにする。
- 候補日程の `YYYY-MM-DD` はUTC日時へ変換せず、年月日を分解してブラウザのローカル日付として扱う。DBの `event_dates` が保持する壁時計時刻と日付境界を一致させ、UTCより遅いタイムゾーンでも前日へずれないようにする。
...
- ヒートマップセルは `0.4rem` の角丸を基本とし、隣接セルと背景色・レイヤー・フィルターが一致する場合は接続辺の角丸を抑制する。これにより、同系色の連続領域は自然につながり、異色境界では角が残って判別しやすくなる。
- 固定のテーブル罫線は使わず、セル内レイヤーの `inset` ラインで境界を描画する。ライン色はセル状態に連動し、ライトテーマでは `0` セルをグレー、可セルを紫系濃色で表示する。
- 上端ライン色は上下セルの境界で判定する。同一 `joinKey`（同色・同効果）の連続領域ではラインを透明化し、境界が変わる箇所のみトーンを切り替えて表示する。
- 境界線は角丸セル内部ではなく `td` 側の上辺ボーダーで描画し、角丸形状に影響されない直線として表示する。
- 色差が明確な境界（例: 紫系セルと白/未回答セル）では境界線を省略し、色面の差だけで区別する。
- 追加で、同トーン同士および `primary/past` の色付きセル同士の境界は線を省略し、連続領域の視覚ノイズを抑える。
- 角丸連結は `joinKey` をトーン単位で判定し、人数差（濃淡差）があっても同系セル同士は連続領域として扱う。
- 例外として、上段セルの `availableCount` が下段セルより多い場合は、上段セルの下角を丸めて段差（上に載る印象）を付与する。
- モバイル列ヘッダーは `曜日 + 月/日` の2段短縮表示を優先し、PCでは従来の最適化日付表示（年/月の省略ロジック）を維持する。セル内は主値（可人数）を優先表示し、狭幅時の一覧性を確保する。
