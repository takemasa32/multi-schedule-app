...
- 週入力由来の値は、既存の各日予定や既回答を上書きするデータとしては扱わない。

## 回答保存時のアカウント予定更新

- ログイン済みユーザーの回答保存では、イベント回答 `availabilities` とアカウント予定 `user_schedule_blocks` を分けて扱う。
- アカウント予定からの自動入力値は、イベント回答画面の初期値としてのみ使う。
- 自動入力されたまま未変更で送信した候補は、アカウント予定へ逆保存しない。
- ユーザーが明示的に変更した候補だけを、アカウント予定の更新対象にする。
- 週入力で新規に補完された候補のうち、もともとイベント回答がなく、各日予定の自動入力でもない候補は、アカウント予定への新規保存対象にする。
- 明示変更された候補は、候補枠を最大1時間単位へ分解して `user_schedule_blocks` へ反映する。
- 長時間候補を明示変更した場合は、候補全体に対するユーザー意思として扱い、分解した各1時間枠へ同じ値で保存する。
- 週入力で新規保存する候補は、既存の `user_schedule_blocks` を更新せず、未保存の1時間枠に対する追加だけを行う。
...
- Step2 の入力内容は現在の回答へ反映する。
- Step2 で反映された候補のうち、未回答かつ各日予定未保存の枠だけは、送信時にアカウント予定へ新規保存する。
