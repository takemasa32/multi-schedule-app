...
- 公開E2Eと認証E2Eは、Playwright設定内でサーバーを共有せず、実行ごとに専用ポート3200 / 3201へ本番ビルドを起動して終了する。既存ポートが使用中なら、そのプロセスを停止せず失敗として通知する。
- 回答画面は、イベント取得後に候補日、既存回答、認証を同時に開始し、同じ認証結果を予定コンテキストへ渡す。
- 予定コンテキスト内の予定ブロック、他イベントの確定日程、現在イベントの上書き情報を並列取得する。
- ローカルDBの実関数に`user_event_links`更新が含まれることを確認し、回答後の重複upsertを削除する。
- ヒートマップの日付ヘッダーで列ごとに再生成していたISO配列を1回の計算へ変更し、曜日フォーマッターも再利用する。
- URL、文言、DOM構造、操作順、テーマ、レスポンシブ表示は変更しない。

日程確定後の利用者別同期、Web Vitals計測、回答ウィザード分割は、整合性・計測設計を伴うため後続の独立変更とする。

### 1.2 検証結果

2026-08-13にWSLとローカルSupabaseで次を確認した。

| 確認項目                 | 結果                                                                       |
| ------------------------ | -------------------------------------------------------------------------- |
| `npm ci` / `npm audit`   | 成功、既知脆弱性0件                                                        |
| `npm ls --depth=0`       | 終了コード0、`invalid`なし。任意のWasm実装は`extraneous`表示のため継続監視 |
| lint / typecheck / build | すべて成功                                                                 |
| Jest                     | 62スイート、360件成功、既存1件skip                                         |
| 公開E2E                  | 20件成功、専用ポート3200を終了後に解放                                     |
| 認証E2E                  | 6件成功、専用ポート3201を終了後に解放                                      |
| ローカルSupabase         | 起動・接続成功。回答保存RPC内の紐付け更新を実関数で確認                    |

`supabase db lint --local`は終了コード0だが、従来どおりpgTAP拡張関数に由来する警告・エラー候補を表示する。アプリ関数の新規指摘ではない。Next.js buildはEdge Runtimeの非推奨警告、Jestは既存テストのReact `act` 警告を表示するため、後続の保守課題として分離する。
