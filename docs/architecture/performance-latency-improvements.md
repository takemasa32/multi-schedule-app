...
- `test:e2e:chrome:public` は手動確認用サーバーを再利用せず、専用ポート3200で本番相当サーバーを毎回起動・終了する。
- 認証付きE2Eも専用ポート3201で独立起動し、公開フローと順番に実行する。
- 専用ポートが使用中の場合は既存プロセスを停止せず、競合理由を表示して終了する。
...
## 2026-07-15: ナビゲーション表示とデータ鮮度の改善

- 静的ページ（トップ・利用規約・プライバシーポリシー・未認可）と、クライアント側で即時に外枠を描画できる履歴ページからルートスケルトンを削除した。
- `/event/[public_id]` の空だったルートローディング表示を、実画面と同じ `app-page`・`surface` 構造へ変更し、画面切り替え時の空白とレイアウトシフトを抑えた。
- `last_accessed_at` の stale 更新は Next.js の `after` でレスポンス後に実行し、イベント本体・候補日・参加者・回答の取得開始を待たせない。
- イベント表示データには TTL や共有の永続キャッシュを導入しない。`React.cache` は同一リクエスト内のイベント重複取得だけに限定し、更新後の再表示では常に Supabase から最新値を取得する。
- 回答・確定・候補日追加など既存の更新処理は `revalidatePath` を継続し、更新直後の画面再取得を保証する。
- 回答入力・日程確定ページでもアクセス時刻更新を `after` へ統一し、表示に必要な取得処理を待たせない。
- `/create` と `/history` はリクエスト固有のサーバーデータを初期描画に使わないため、`force-dynamic` を外して静的シェルを即時配信する。
- 回答入力・日程確定には各画面の実レイアウトに対応する専用スケルトンを追加し、イベント詳細用スケルトンが流用される違和感をなくした。
- 回答入力からイベント詳細へ戻る導線は通常の `<a>` から Next.js の `<Link>` へ変更し、クライアントナビゲーションとプリフェッチを利用する。
- レスポンス後のアクセス時刻更新を `deferEventLastAccessedTouch` に集約し、イベント詳細・回答入力・日程確定で同じライフサイクルを保証する。
- ルートスケルトンのコンテナと読み込み状態通知を `PageLoadingShell` に集約し、各画面固有の骨格だけを個別に管理する。
