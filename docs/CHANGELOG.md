...
- 3時間以上の候補で、アカウント予定から集約して自動入力された値を未変更のまま送信しても、保存元の1時間予定へ逆保存しないよう修正。
- 回答送信時のアカウント予定保存対象を、ユーザーが明示的に変更した候補だけに限定する仕様へ変更。
- `availability-form` から `edited_date_ids` を送信し、`submit_availability_bundle` はその候補だけを1時間単位へ展開して `user_schedule_blocks` へ反映するよう更新。
- 曜日一括入力で未回答かつ各日予定未保存の枠に新規適用された候補は、送信時に `weekly_applied_date_ids` として渡し、既存ブロックを上書きせずに `user_schedule_blocks` へ新規保存するよう変更。
- 自動入力未変更送信では元の `10-11 / 11-12 / 12-13` 予定が壊れないこと、および明示変更時のみ長時間候補全体を反映することを検証する pgTAP / 単体テストを追加。
