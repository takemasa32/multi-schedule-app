...
LP以外の主要画面は、改修時にデスクトップ／モバイルの比較画像を
`docs/screenshots/uiux-refresh/` へ保存します。表示確認では、作成・回答・集計・確定の主要導線とライト／ダークテーマを確認してください。

イベントフローの比較画像を更新する場合は、開発サーバーを起動したうえで次を実行します。
