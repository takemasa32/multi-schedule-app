...
describe('AvailabilityForm', () => {
  ...
  it('自動入力だけで未変更送信した場合は edited_date_ids を空で送る', async () => {
    render(
      <AvailabilityForm
        {...defaultProps}
        mode="new"
  ...
  it('ユーザーが候補を明示変更した場合は edited_date_ids に含めて送る', async () => {
    ...
    const heatmapCell = document.querySelector<HTMLElement>('[data-selection-key="date2"]');
    if (!heatmapCell) throw new Error('ヒートマップセルが見つかりません');
    fireEvent.click(heatmapCell);
    fireEvent.click(screen.getByRole('button', { name: 'この枠を選択する' }));
    await waitFor(() => {
      expect(heatmapCell).toHaveTextContent('○');
    });
