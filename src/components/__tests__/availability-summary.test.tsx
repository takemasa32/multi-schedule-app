...
describe('AvailabilitySummary', () => {
  ...
  it('参加者がいない場合はメッセージを表示する', () => {
    render(<AvailabilitySummary {...defaultProps} participants={[]} availabilities={[]} />);
    // 全員が除外された場合は何も表示されない（メッセージも表示されない）
    expect(screen.queryByText(/表示中の参加者はいません/)).not.toBeInTheDocument();
  });
  ...
  describe('excludedParticipantIds', () => {
    it('指定された参加者が除外される', () => {
      // Alice（p1）を除外する
      render(<AvailabilitySummary {...defaultProps} excludedParticipantIds={['p1']} />);
    ...
    it('複数の参加者が除外される', () => {
      // AliceとBobを除外する
      render(<AvailabilitySummary {...defaultProps} excludedParticipantIds={['p1', 'p2']} />);
  ...
  it('全ての参加者が除外された場合は適切なメッセージが表示される', () => {
    // 全員を除外する
    render(<AvailabilitySummary {...defaultProps} excludedParticipantIds={['p1', 'p2', 'p3']} />);
  ...
  it('excludedParticipantIdsが空配列の場合は全員が表示される', () => {
    render(<AvailabilitySummary {...defaultProps} excludedParticipantIds={[]} />);
  ...
  it('存在しない参加者IDが除外リストにあってもエラーにならない', () => {
    render(<AvailabilitySummary {...defaultProps} excludedParticipantIds={['nonexistent-id']} />);
  ...
  describe('excludedParticipantIds 機能', () => {
    it('単一参加者を除外した場合、集計から除外される', () => {
      ...
      expect(screen.getByTestId('available-count-date1')).toHaveTextContent('1');
      expect(screen.getByTestId('unavailable-count-date1')).toHaveTextContent('1');

      // date2: Alice(×)除外 → Bob(○), Charlie(○) = ○2名/×0名
      expect(screen.getByTestId('available-count-date2')).toHaveTextContent('2');
      expect(screen.getByTestId('unavailable-count-date2')).toHaveTextContent('0');
    });
    ...
    it('複数参加者を除外した場合、集計から除外される', () => {
      ...
      expect(screen.getByTestId('available-count-date1')).toHaveTextContent('1');
      expect(screen.getByTestId('unavailable-count-date1')).toHaveTextContent('0');

      // date2: Charlieのみ(○) = ○1名/×0名
      expect(screen.getByTestId('available-count-date2')).toHaveTextContent('1');
      expect(screen.getByTestId('unavailable-count-date2')).toHaveTextContent('0');
    });
