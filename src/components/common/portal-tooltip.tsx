...
export default function PortalTooltip({ open, anchor, text, onClose, maxWidth = 320 }: Props) {
  const rootEl = useMemo(() => {
    if (typeof document === 'undefined') return null;
    let elem = document.getElementById('global-portal-tooltip') as HTMLDivElement | null;
    if (!elem) {
  ...
  return createPortal(
    <div
      ...
      className="bg-base-100 border-base-300 break-words rounded-md border p-3 text-sm shadow-lg"
