...
本アプリでは、サービス名・キャッチフレーズ・説明文・OGP画像など、サイト全体で利用する情報を一元管理するための仕組みを導入しています。これにより、サービス名称や説明文の変更、SEO最適化、UI表示の統一が容易になります。
...
- 各ページやコンポーネント、OGP生成、SEO設定などでこの site-config.ts を参照します。

## 4. メリット

- サービス名や説明文の変更が1ファイルの修正で全体に反映される
- SEO・OGP・UI表示の一貫性が保たれる
...
本仕様は、サービス名称・説明文・OGP などのサイト情報を一元管理し、保守性・一貫性・拡張性を高めるための実装方針を示しています。
