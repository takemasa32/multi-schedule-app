...
- 説明は日本語で記述してください。
- GEMINI CLI のカスタム指示は、GEMINI CLI の動作を制御するための設定です。
...
- このリポジトリではテストを利用できます
  - 単体テストは、`npm run test:jest` コマンドで実行できます。
  - E2E テストは、`npm run test:e2e:chrome` コマンドで実行できます。
  - テストの追加や修正は、既存のテストコードを参考にしてください。
